...
// Samples an element in Z with probability proportionnal to
// e^{-x^2/2*(sigma^2)}
//==============================================================================
signed int Sample3(const RR_t sigma128) {
...

    {
        ...
        borne = exp(-x * x * (1 / (2 * sigma * sigma) -
                              1 / (2 * k * k * sigma_1 * sigma_1)));
        assert(borne <= 1);
        if (alea < borne) {
            return x;
        }
...
signed int Sample4(RR_t c, RR_t sigma) {
    ...
    while (1) {
        ...
        coin >>= 1;
        borne = exp(-(x - fracc) * (x - fracc) * denom) /
                (exp(-x * x * denom) + exp(-(x - 1) * (x - 1) * denom));
