...
void FFTStep(CC_t* const f_fft, RR_t const* const f, const unsigned long N,
             const CC_t w0) {
    if (N == 1) {
        f_fft[0] = f[0];
    } else {
...
void ReverseFFTStep(CC_t* const f, CC_t const* const f_fft,
                    const unsigned long N, const CC_t w0) {
    if (N != 2) {
        assert(N % 2 == 0);
